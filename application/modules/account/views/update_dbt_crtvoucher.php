    ...
    <div class="col-sm-12 col-md-12">
        <div class="panel panel-bd">
            ...
            <div class="panel-body">
                <?php echo  form_open_multipart('account/accounts/update_debit_voucher') ?>
                 <div class="form-group row">
                    <label for="vo_no" class="col-sm-2 col-form-label"><?php echo display('voucher_no')?>*</label>
                    <div class="col-sm-4">
                       <input type="text" name="txtVNo" required id="txtVNo" value="<?php echo $vNo;?>" class="form-control" readonly /> 
                   ...
                   foreach ($voucher_info as $voucher){ ?>                           
                     <tr>
                        ...
                        <?php if( $voucher->subType  != 1) {
                         ...
                         <td >  
                            <select name="subtype[]" id="subtype_<?php echo $sl; ?>" required class="form-control">
                              <?php foreach ($subtpes as $subtpe) {?>
                                 <option value="<?php echo $subtpe->id;?>" <?php echo $voucher->subType == $subtpe->id?'selected' :'';?> ><?php echo $subtpe->name;?></option>
                             <?php } ?>
                            </select>
                        ...
                        <td><input type="number" name="txtAmount[]" required value="<?php echo $voucher->Debit;?>" class="form-control total_price text-right"  id="txtAmount_<?php echo $sl; ?>" onkeyup="calculationDebtv(<?php echo $sl; ?>)" >
                           </td>
                       <td>
