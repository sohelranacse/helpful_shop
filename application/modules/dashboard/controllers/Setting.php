...
class Setting extends MX_Controller {
	...
	public function bdtask_create_settings()
	{
		...
		if($this->input->post('is_qr',true) == 1){
			$is_qr = 1;
		}else{
			$is_qr = 0;
		}
		if($this->input->post('is_autoapprove_v',true) == 1){
			$is_autoapprove_v = 1;
		}else{
			$is_autoapprove_v = 0;
		}
	...
	// warehouse
	function warehouse($warehouse_id = null) {
		$data['title']    = display('warehouse');

		if(!empty($_POST)) {
			$this->form_validation->set_rules('warehouse_name', display('warehouse_name'),'required|max_length[155]');
			if ($this->form_validation->run()) {
				if (empty($this->input->post('warehouse_id'))) {
					if ($this->setting_model->add_warehouse()) {
						$this->session->set_flashdata('message', display('save_successfully'));
					} else {
						$this->session->set_flashdata('exception', display('please_try_again'));
					}
					...
					if ($this->setting_model->update_warehouse()) {
						$this->session->set_flashdata('message', display('update_successfully'));
					} else {
						$this->session->set_flashdata('exception', display('please_try_again'));
					}

					redirect("warehouse/$warehouse_id");
				}
			}
		}

		if(!empty($warehouse_id)){ // edit
			$data['title']    = display('edit_warehouse');
			$data['warehouse_data'] = $this->setting_model->warehouse_editdata($warehouse_id);
		}
		$data['module']   = "dashboard";  
		$data['page']     = "warehouse/index";
		$data['warehouse_list'] = $this->setting_model->warehouse_list();

		echo Modules::run('template/layout', $data);
	}
	public function delete_warehouse($id = null) {
		if ($this->setting_model->delete_warehouse($id)) {
