...
class Invoice_model extends CI_Model {
    ...
    function invoice_no_generator() {
        $query = $this->db->query("SELECT MAX(invoice) AS invoice_no FROM invoice");        
    ...
    public function insert_invoice() {
        ...
        $data = array(
            ...
            'invoice'           => $this->invoice_no_generator(),
        ...
        foreach($product_id_A as $key => $product_id) {
            $data1 = array(
                ...
                'discount_per'      => $discount_per[$key],
                'discount'          => $discountvalue[$key],
                'status'            => 1
            );

            $this->db->insert('invoice_details', $data1);
        }
        return $invoice_insert_id;
    }
    function invoice_info($id) {
        $query = $this->db->query("
            SELECT *
            FROM invoice a
            INNER JOIN customer_information b ON (a.customer_id = b.customer_id)
            WHERE id = $id
        ");
        return $query->row();
    }
    function invoice_details($id) {
        $query = $this->db->query("
            SELECT a.*, b.product_name, c.unit_name
            FROM invoice_details a
            INNER JOIN product_information b ON (a.product_id = b.product_id)
            INNER JOIN units c ON (b.unit = c.unit_id)
            WHERE invoice_id = $id
        ");
        return $query->result();
    }
    public function update_invoice_save() {
        ...
        if($this->permission1->method('manage_invoice','update')->access()){
          ...
          $details ='  <a href="'.$base_url.'invoice_details/'.$record->invoice_id.'" class="" >'.$record->invoice.'</a>';
            ...
            $data[] = array( 
                ...
                'delivery_note'    =>$record->invoice_details,
