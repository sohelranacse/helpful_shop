...
class Product extends MX_Controller {
    ...
    // writer
    function writer($writer_id = null) {
        $data['title']    = display('writer');

        if(!empty($_POST)) {
            $this->form_validation->set_rules('writer_name', display('writer_name'),'required|max_length[155]');
            $this->form_validation->set_rules('status', display('status'),'required|max_length[1]');
            if ($this->form_validation->run()) {
                if (empty($this->input->post('writer_id'))) {
                    if ($this->product_model->add_writer()) {
                    ...
                    if ($this->product_model->update_writer()) {
                        $this->session->set_flashdata('message', display('update_successfully'));
                    } else {
                        $this->session->set_flashdata('exception', display('please_try_again'));
                    }

                    redirect("writer/$writer_id");
                }
            }
        }

        if(!empty($writer_id)){ // edit
            $data['title']    = display('edit_writer');
            $data['writer_data'] = $this->product_model->writer_editdata($writer_id);
        }
        $data['module']   = "product";  
        $data['page']     = "writer";
        $data['writer_list'] = $this->product_model->writer_list();

        echo Modules::run('template/layout', $data);
    }
    public function delete_writer($id = null) {
        if ($this->product_model->delete_writer($id)) {
    ...
    // publication
    function publication($publication_id = null) {
        $data['title']    = display('publication');

        if(!empty($_POST)) {
            $this->form_validation->set_rules('publication_name', display('publication_name'),'required|max_length[155]');
            $this->form_validation->set_rules('status', display('status'),'required|max_length[1]');
            if ($this->form_validation->run()) {
                if (empty($this->input->post('publication_id'))) {
                    if ($this->product_model->add_publication()) {
                        $this->session->set_flashdata('message', display('save_successfully'));
                    } else {
                        $this->session->set_flashdata('exception', display('please_try_again'));
                    }
                    ...
                    if ($this->product_model->update_publication()) {
                        $this->session->set_flashdata('message', display('update_successfully'));
                    } else {
                        $this->session->set_flashdata('exception', display('please_try_again'));
                    }

                    redirect("publication/$publication_id");
                }
            }
        }

        if(!empty($publication_id)){ // edit
            $data['title']    = display('edit_publication');
            $data['publication_data'] = $this->product_model->publication_editdata($publication_id);
        }
        $data['module']   = "product";  
        $data['page']     = "publication";
        $data['publication_list'] = $this->product_model->publication_list();

        echo Modules::run('template/layout', $data);
    }
    public function delete_publication($id = null) {
        if ($this->product_model->delete_publication($id)) {
            $this->session->set_flashdata('message', display('delete_successfully'));
        } else {
            $this->session->set_flashdata('exception', display('please_try_again'));
        }
    ...
    public function product_form($id = null){
        ...
        $this->form_validation->set_rules('supplier_price', display('cost_price') ,'required|max_length[12]');
        $this->form_validation->set_rules('price', display('price') ,'required|max_length[12]');
        $this->form_validation->set_rules('supplier_id', display('supplier') ,'required|max_length[12]');
        ...
        $data['product'] = (object)$postData = [
            ...
            'publication_id'    => $this->input->post('publication_id',TRUE),
        ...
        if ($this->form_validation->run() === true) {
            ...
            if (empty($id)) { // insert
                ...
                $postData['added_by'] = $this->session->userdata('id');
                ...
                $postData['edited_on'] = date('Y-m-d H:i:s');
                $postData['edited_by'] = $this->session->userdata('id');
            ...
            $data['writer_list'] = $this->product_model->active_writer();
            $data['publication_list'] = $this->product_model->active_publication();
    ...
    function uploadCsv()
    {
        ...
        $product_info = $this->product_model->barcode_productdata($product_id);
        $data = array(
...
public function product_details($product_id = null){
        $details_info = $this->product_model->barcode_productdata($product_id);
