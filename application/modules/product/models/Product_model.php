...
class Product_model extends CI_Model {
    ...
    public function add_writer(){
        ...
        return $this->db->insert('writer',$data);
    }
    ...
    public function update_writer()
    {
        ...
        return $this->db->where('writer_id',$this->input->post('writer_id'))
            ->update('writer',$data); 
    } 

    public function writer_editdata($writer_id){
        $data = $this->db->select("*")
            ->from('writer')
    ...
    public function update_publication()
    {
        ...
        return $this->db->where('publication_id',$this->input->post('publication_id'))
            ->update('publication',$data); 
    } 

    public function publication_editdata($publication_id){
        $data = $this->db->select("*")
            ->from('publication')
    ...
    public function active_publication() {
        $this->db->select('*');
    ...
    public function getProductList($postData=null){
        ...
        $this->db->join('supplier_information m','m.supplier_id = a.supplier_id','left');
        if($searchValue != '')
        ...
        foreach($records as $record ){
            ...
            if($this->permission1->method('manage_product','delete')->access()) {
                ...
                $button .= '<a href="'.$base_url.'product/product/deleteproduct/'.$record->product_id.'" class="btn btn-xs btn-danger "  onclick="'.$jsaction.'"><i class="fa fa-trash"></i></a>';
            }
    ...
    public function barcode_productdata($id){
         return $this->db->select('*')
            ->from('product_information')
            ->where('product_id', $id)
            ->get()
