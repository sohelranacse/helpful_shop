...
class Report extends MX_Controller {
    ...
    function opening_stock() {
        if(!empty($_POST)) {
            $this->form_validation->set_rules('warehouse_id', 'warehouse_id', 'trim|required|numeric');
            $this->form_validation->set_rules('product_id', 'product_id', 'trim|numeric');
            $this->form_validation->set_rules('quantity', 'quantity', 'trim|numeric');
            if($this->form_validation->run() == TRUE) {
                $this->report_model->add_opening_stock();
                $this->session->set_flashdata('message', display('save_successfully'));
            }
            redirect('opening_stock');
        }

        $data['title']      = display('opening_stock');
        $data['warehouse_list'] = $this->report_model->warehouse_list();
        $data['module']     = "report";
        $data['page']       = "opening_stock"; 
        echo modules::run('template/layout', $data);
    }
    function get_item_list_by_warehouse($warehouse_id) {
        $data['warehouse_id'] = $warehouse_id;
        $data['item_list'] = $this->report_model->get_item_list_by_warehouse($warehouse_id);
        $this->load->view('report/get_item_list_by_warehouse',$data);
    }
    /*product stock part*/
