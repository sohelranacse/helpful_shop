                ...
                <div class="panel panel-bd lobidrag">
                    ...
                    <div class="panel-body">
                        <div class="table-responsive">
                            <table id="dataTableExample" class="table datatable table-bordered table-striped table-hover">
                                <thead>
                                    <tr>
                            ...
                            <td class="text-center"><?php echo html_escape($services['details']);?></td>
                                    ...
                                    <center>
                                        ...
                                        <?php if($this->permission1->method('manage_service','update')->access()){ 
                                            ...
                                            $approve = $this->db->select('status,referenceNo')->from('acc_vaucher')->where('referenceNo', $services['voucher_no'])->where('status', 1)->get()->num_rows();
                                            if ($approve == 0) {
                                            ?>
                                    ...
                                    <?php }}?>
                                      <?php if($this->permission1->method('manage_service','delete')->access()){ 
                                        if ($approve == 0) {?>
                                        <a href="<?php echo base_url() . 'service/service/service_invoic_delete/'.$services['voucher_no'].'/'.$services['id']; ?>" class="btn btn-danger btn-sm" name="<?php echo $services['voucher_no'];?>" onclick="return confirm('<?php echo display('are_you_sure') ?>')" data-original-title="<?php echo display('delete') ?> "><i class="fa fa-trash-o" aria-hidden="true"></i></a>
                                    <?php }}?>
                                     <a href="<?php echo base_url() . 'service/service/servicedetails_download/'.$services['voucher_no']; ?>" class="btn btn-info btn-sm" data-toggle="tooltip" data-placement="left" title="<?php echo display('download') ?>"><i class="fa fa-download" aria-hidden="true"></i></a>
                                      ...
                                      <a href="<?php echo base_url() . 'service_details/'.$services['voucher_no']; ?>" class="btn btn-success btn-sm" name="<?php echo $services['voucher_no'];?>"  title="<?php echo display('details'); ?> "><i class="fa fa-eye" aria-hidden="true"></i></a>
                                            <?php echo form_close() ?>
                                    </center>
                                    </td>
                                    </tr>
