...
<div class="content-wrapper">
    <section class="content-header">
        ...
        <div class="header-title">
            <h1><?php echo display('add_office_loan') ?></h1>
            <small><?php echo display('add_office_loan') ?></small>
            <ol class="breadcrumb">
                <li><a href="#"><i class="pe-7s-home"></i> <?php echo display('home') ?></a></li>
                <li><a href="#"><?php echo display('personal_loan') ?></a></li>
                <li class="active"><?php echo display('add_office_loan') ?></li>
            </ol>
        </div>
    </section>
        ...
        <div class="row">
            <div class="col-sm-12">
                ...
                <?php if($this->permission1->method('add1_person','create')->access()){ ?>
                   <a href="<?php echo base_url('Cloan/add_person')?>" class="btn btn-info m-b-5 m-r-2"><i class="ti-plus"> </i> <?php echo display('add_person')?> </a>
               <?php }?>
        <?php if($this->permission1->method('add_loan_payment','create')->access()){ ?>
                  <a href="<?php echo base_url('Cloan/add_loan_payment')?>" class="btn btn-success m-b-5 m-r-2"><i class="ti-plus"> </i> <?php echo display('add_payment')?> </a>
               <?php }?>
               <?php if($this->permission1->method('manage1_person','read')->access()){ ?>
                  <a href="<?php echo base_url('Cloan/manage1_person')?>" class="btn btn-primary m-b-5 m-r-2"><i class="ti-plus"> </i> <?php echo display('manage_loan')?> </a>
                  <?php }?>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
    ...
    $('body').on('change','#name',function(event){
        ...
        $.ajax({
            url: '<?php echo base_url('Cloan/loan_phone_search_by_name')?>',
            type: 'post',
